/*
  ESP32 Wi-Fi Setup Portal + IR (Arduino-IRremote) + HTTP API + Web UI + mDNS (unique per device)
...
// ---- IRremote: define pin before including IRremote.hpp ----
...
String portalRedirectPage(const String& ssid, const String& targetUrl) {
  ...
  html += "<script>"
          ...
          "  try {"
          "    // If host is reachable, this resolves (even if CORS blocks reading)."
...
String staIndexPage() {
  ...
  // Escape anything that goes into HTML
...
void setupPortalRoutes() {
  ...
  server.on("/save", HTTP_POST, []() {
    ...
    // Save and compute the final unique mDNS name immediately (for the redirect page)
...
void setup() {
  ...
  // Load saved settings and compute names
  loadSettings();
